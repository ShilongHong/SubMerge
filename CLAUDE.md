...
The frontend has no build step. Any local `package.json`, `tailwind.config.js` or `tailwind-input.css` are gitignored leftovers from the old Tailwind build and unused.
...
`GET /api/check/<token>` does not merge. It fetches each saved subscription on its own (`download_subscription(prefer_cache=True)`, or the uploaded file) and returns per-subscription `ok`, `nodes`, `traffic` and `error`, so the config page can show which source is broken. `?refresh=1` forces a download.
...
- The template's AI rules are split three ways: `🧲 OpenAI` (ChatGPT/OpenAI and its third-party services), `🧲 Claude` (Anthropic, `DOMAIN-KEYWORD` rules for `anthropic`/`claude`, and the old default list's broad `storage.googleapis.com`, `cdn.cloudflare.net`, `analytics.google.com`, `amplitude.com`) and `🧲 海外AI` (Gemini/Google AI, openrouter, Cursor). The AI groups take precedence: rules in other groups that duplicate or are covered by an AI rule were deleted, and `gemini.gstatic.com` sits before `📥 下载`'s `gstatic.com`. Keep it that way when editing (`sentry.io` stays with OpenAI because it comes first).
...
- `templates/index.html`: the `#subTemplate` card markup
- `static/app.js`: `addSub()` (fill from saved data), `collect()`, `snapshot()` and the `formData.append` block in `submit()` (form fields are indexed, e.g. `in_rules_N`)
- `app.py`: form parsing in both `create_subscription()` and `update_config()`, plus the saved dict in `create_subscription()`

### Frontend

- The config page is `templates/index.html` (markup and the subscription card `<template>`), `static/app.css` (plain CSS; light/dark tokens on `:root`, `data-theme` overrides the system setting) and `static/app.js` (vanilla JS, no dependencies, no CDN).
- It is a three-step flow: 订阅源 cards → 合并设置 (two selects that set `is_main` / `is_traffic_main`, so exclusivity is structural) → result panel with V1/V2 links, the token and the `/api/check` results. Validation mirrors backend constraints (unique names, no commas, no reserved group names like `节点信息`/`SuperSub`, no `_Auto` suffix).
- After saving, the page re-reads `/api/config/<token>` to pick up new `file_md5` values. `/?token=…` auto-loads a config; recent tokens are kept in `localStorage` (`submerge-recent`).
- `.impeccable.md` is the design brief (audience, tone, layout, accessibility requirements). Follow it for UI work.
...
- `AGENTS.md`, `utils/AGENTS.md` and `.github/copilot-instructions.md` are outdated. They describe custom-rule injection, `validate_clash_rule`, multi-User-Agent retries and a PyInstaller `build.bat`/`build.spec`, and none of these exist. Trust the code.
